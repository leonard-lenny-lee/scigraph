...
__all__ = ["VarType", "MultipleVariablesTable"]
...
class MultipleVariablesTable(DataTable):
    ...
    def __init__(
        ...
        variable_types: Optional[List[VarType]] = None,
    ) -> None:
    ...
    def get_variable_types(self) -> Dict[str, VarType]:
        ...
        """
        return dict(zip(self.variable_names, self.var_types))
        ...
        if self.var_types is None:
            ...
            for column in self.data.columns:
                try:
                    ...
                    variable_types.append(VarType.CONTINUOUS)
                except ValueError:
                    self.data[column] = self.data[column].astype("category")
                    variable_types.append(VarType.CATEGORICAL)
            self.var_types = variable_types
            return

        # If variable types are provided, validate and coerce
        if len(self.var_types) != self.n_variables:
            raise ValueError("Each variable must be assigned a single type")

        for var_type, column in zip(self.var_types, self.data.columns):
            ...
            if var_type is VarType.CONTINUOUS:
                self.data[column] = to_numeric(self.data[column])
            elif var_type is VarType.CATEGORICAL:
                self.data[column] = self.data[column].astype("category")
            else:
                assert var_type is VarType.LABEL
